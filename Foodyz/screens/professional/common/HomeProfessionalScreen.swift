    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            VStack(spacing: 0) {
                ...
                FoodyzTopBar(path: $path, professionalId: professionalId)
...
struct FoodyzTopBar: View {
    ...
    var body: some View {
        HStack {
            ...
            Button(action: {
                // TODO: Add menu navigation when Screen.menu is defined
                print("Menu tapped")
            }) {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ForEach(Array(icons.enumerated()), id: \.offset) { index, icon in
                ...
                Button(action: {
                    if icon.systemName == "plus.square.fill" {
                        // Navigate to professional add content screen
                        path.append(Screen.professionalAddContent)
                    } else if icon.systemName == "list.bullet.rectangle.fill" {
                        // TODO: Add menu navigation when Screen.menu is defined
                        print("Menu tapped")
                    } else {
